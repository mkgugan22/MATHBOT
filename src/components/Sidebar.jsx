...
export default function Sidebar({ onThemeChange }) {
  ...
  const confirmEdit = (e) => {
    e.stopPropagation();
    if (editTitle.trim()) {
      dispatch(renameSession({ sessionId: editingId, title: editTitle.trim() }));
    }
    setEditingId(null);
  };
                    ...
                    {editingId === session.id ? (
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }} onClick={e => e.stopPropagation()}>
                        <input
                          ...
                          onKeyDown={e => e.key === 'Enter' && confirmEdit(e)}
                          autoFocus
                          style={{
                            flex: 1, background: '#1a1a3a', border: '1px solid #6366f1',
                            color: '#fff', borderRadius: 6, padding: '4px 8px',
                            fontSize: 12, fontFamily: "'Syne', sans-serif", outline: 'none',
                          }}
                        />
                      ...
                      <>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4, alignItems: 'center' }}>
                          <span style={{ fontSize: 10, color: '#3a3a6a', fontFamily: "'JetBrains Mono', monospace" }}>
